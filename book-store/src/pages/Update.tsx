...
export default function Add() {
    ...
    const mutation = useMutation({
        mutationFn:(formData:books)=>{
            return axios.put(`http://localhost:4242/books/${id}`,formData).then(res=>console.log(res)).catch(err=>console.log(err));
        }
    })
  ...
  return (
    <div className='flex flex-col gap-5 items-center justify-center h-[100vh]'>
        <h2 className='text-4xl  font-semibold'>Upate your books</h2>
        <form action="" className='flex flex-col gap-2 w-11/12 max-w-[450px]' onSubmit={handleSubmit}>
            ...
            <button type='submit' className='px-4 py-2 rounded-sm bg-violet-700 text-white'>Update</button>
        </form>
    </div>
  )
}
